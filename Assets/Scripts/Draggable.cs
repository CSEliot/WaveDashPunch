...
public class Draggable : MonoBehaviour
{
    ...
    private void Start()
    {
        colorCam = GameObject.FindGameObjectWithTag("ColorCam").GetComponent<Camera>();
    }
    ...
    void Update()
	{
		if (Input.GetMouseButtonDown(0)) {
			dragging = false;
			var ray = colorCam.ScreenPointToRay(Input.mousePosition);
			RaycastHit hit;
		...
		if (dragging && Input.GetMouseButton(0)) {
			var point = colorCam.ScreenToWorldPoint(Input.mousePosition);
